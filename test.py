...
def plot_confusion_matrix(cm,
    ...
    plt.savefig('./result/AlexNet_Mirror.jpg', bbox_inches='tight')
...
def test():
    ...
    model.load_state_dict(torch.load('./result/AlexNet_Mirror_model.pth'))
    model.eval()
    ...
    with torch.no_grad():
        ...
        with open('./result/AlexNet_Mirror_roc.pkl', "wb") as fw:
            pickle.dump((fpr, tpr, AUC), fw)
        with open('./result/AlexNet_Mirror_roc.pkl', "rb") as fr:
            FPR, TPR, AUC = pickle.load(fr)

        plt.figure(figsize=(8, 6))
        plt.plot(FPR, TPR, color='red', linewidth=2, label='AlexNet  AUC = %.4f' % AUC)
